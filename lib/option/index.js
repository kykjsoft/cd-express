...
const {mergeOption} = require("../merge")
...
const defaultSetting = {
    log:false,
    file:"cd-express.json"
}

async function init(cwd,opt){
    if(!cwd){ throw new Error("必须传入目录地址");}

    opt = mergeOption({},defaultSetting,opt)

    cwd=cwd||process.cwd;
    const cfgfilename = await hasConfig(cwd,opt);
    if(cfgfilename){
        isDebug && console.log(`已存在配置文件 ${cfgfilename}`);
        opt.error = `已存在配置文件 ${cfgfilename}`;
        return opt;
    }
    ...
    try{
        ...
        await createConfigFile(cwd,merge(await defaults(),config),opt.file);
        isDebug && console.log(`已生成配置文件 ${opt.file}`);
        isDebug && console.log(`执行 cd-express${opt.file===defaultSetting.file?"":" -o "+opt.file} 即可运行`);
        
        opt.error = null;
        return opt;
    }catch(ex){
        isDebug && console.log("配置文件生成失败",ex);
        opt.error = ex.message || "配置文件生成失败";
    }
    return opt;
}
...
async function hasConfig(cwd,opt){
    ...
    if(opt.file !== defaultSetting.file){
        try{
            const cfgpath = join(cwd,opt.file);
            await access(cfgpath);
            return cfgpath
        }catch(ex){
            opt.log && console.log(`读取配置文件 ${opt.file} 失败`);
            return null;
        }
    }
    ...
    try{
        const cfgpath = join(cwd,defaultSetting.file);
        await access(cfgpath);
        return cfgpath
    }catch(ex){
        opt.log && console.log("读取配置文件 "+ defaultSetting.file +" 失败");
    }
    return null;
}

async function getConfig(cwd,opt){
    if(!cwd){ throw new Error("必须传入目录地址");}
    console.log(opt)
    opt = mergeOption({},defaultSetting,opt)
    ...
    if(opt.file!== defaultSetting.file){
        try{
            return await getJSON5Config(join(cwd,opt.file))
        }catch(ex){
            throw `配置文件 [ ${opt.file} ] 不存在`
        }
    }
    try{
        return await getJSON5Config(join(cwd,opt.file))
...
async function createConfigFile(cwd,option,file=defaultSetting.file){
    if(!cwd){ throw new Error("必须传入目录地址");}
